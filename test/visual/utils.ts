...
// Wait for Tech Stack page to be fully loaded and ready for screenshot
export async function waitForTechStackPage(page: Page): Promise<void> {
  await page.waitForLoadState("networkidle");

  // Wait for fonts to load
  await page.evaluate(() => document.fonts.ready);

  // Wait for Tech Stack heading to be visible
  await page.waitForSelector("h1", { state: "visible" });
